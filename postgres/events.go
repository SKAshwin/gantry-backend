...
func (es *EventService) EventsBy(username string) ([]checkin.Event, error) {
	...
	rows, err := es.DB.Queryx("SELECT id, name, \"start\", \"end\", lat, long, radius, url, updatedat, createdat, timetags, website, identifier from event, hosts where hosts.username = $1 and hosts.eventID = event.ID",
		username)
	if err != nil {
...
func (es *EventService) CreateEvent(e checkin.Event, hostUsername string) error {
	...
	_, err = tx.NamedExec("INSERT INTO event(id, name, url, start, \"end\", timetags, lat, long, radius, website, identifier) VALUES (:id, :name, :url, :start, :end, :timetags,:lat, :long, :radius, :website, :identifier)", rawEvent)
	if err != nil {
		tx.Rollback()
...
func (es *EventService) UpdateEvent(event checkin.Event) error {
	...
	res, err := es.DB.NamedExec("UPDATE event SET name = :name, timetags = :timetags, \"start\" = :start, "+
		"\"end\" = :end, lat = :lat, long= :long, radius = :radius, url = :url, website = :website, identifier = :identifier, updatedAt = (NOW() at time zone 'utc') where id = :id",
		&rawEvent)
	if err != nil {
